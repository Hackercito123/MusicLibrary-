...
class GenreBST:
    ...
    def _build_tree(self, hierarchy, parent=None):
        ...
        node.left = self._build_tree({k: v for k, v in hierarchy.items() if k < genre}, node)
        node.right = self._build_tree({k: v for k, v in hierarchy.items() if k > genre}, node)
    ...
    def _search(self, node, genre):
        ...
        if genre == node.genre:
            return node.songs + self._search_subgenres(node.left, genre) + self._search_subgenres(node.right,
        genre)
        elif genre < node.genre:
    ...
    def _search_subgenres(self, node, parent_genre):
        ...
        return songs + self._search_subgenres(node.left, parent_genre) + self._search_subgenres(node.right,parent_genre)
