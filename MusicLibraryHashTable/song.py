class Song:
  def __init__(self, title, artist, score=0, genre="", danceability=0.0, energy=0.0, valence=0.0, duration=0):
    self.title = title
    self.artist = artist
    self.score = score
    self.genre = genre
  ...
  def __str__(self):
    return (f"{self.title[:22]+'...' if len(self.title) > 25 else self.title:25} "
