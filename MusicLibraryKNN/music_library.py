...
class Graph:
    ...
    def add_edge(self, song_from, song_to):
        ...
        self.graph[song_to].append(song_from) # Make graph uniderectional
    ...
    def calculate_weight(self, song1, song2):
        ...
        weight = (danceability_diff * 3 + energy_diff * 3 + valence_diff * 3 + duration_diff + score_diff) / 11
        return weight
    ...
    def display_graph(self):
        ...
        for song, connections in self.graph.items():
            conn_details = [f"{conn} (weight: {self.weights.get((song, conn), 1):.2f})" for conn in connections]
...
class MusicLibrary:
    ...
    def __init__(self):
        ...
        self.graph = Graph() # Initialize graph
      ...
      for i in range(1, num_songs + 1):
         for w in range(available_time + 1):
           if self.songs[i-1].duration <= w:
            dp[i][w] = max(self.songs[i-1].score + dp[i-1][w - self.songs[i-1].duration], dp[i-1][w])
         else:
            dp[i][w] = dp[i-1][w]
    ...
    def display_knn(self, target_song_title, k=5):
       ...
       if nearest_neighbors:
         ...
         print(f"No nearest neighbors found or song '{target_song_title}' is not in the library.")
